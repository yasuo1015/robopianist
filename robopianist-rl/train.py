...
def _get_task(env):
    """Unwrap to find the composer env with task attribute."""
    e = env
    while e is not None:
        if hasattr(e, "task"):
            return e.task
        e = getattr(e, "_environment", None)
    return None
...
def get_env(args: Args, record_dir: Optional[Path] = None):
    env = suite.load(
        ...
        task_kwargs=dict(
            ...
            enable_velocity_reward=args.enable_velocity_reward,
            velocity_reward_start_step=args.velocity_reward_start_step,
        ),
    )
    ...
    if args.enable_velocity_reward and args.velocity_reward_start_step > 0:
        task = _get_task(env)
        env = robopianist_wrappers.VelocityRewardCurriculumWrapper(
            env, task, args.velocity_reward_start_step
        )
    return env
...
def main(args: Args) -> None:
    if args.name:
        ...
        base = f"SAC-{args.environment_name}-{args.seed}-{time.time()}"
        run_name = f"{base}-have" if args.enable_tactile else f"{base}-not"
    ...
    for i in tqdm(range(1, args.max_steps + 1), disable=not args.tqdm_bar):
        ...
        if i % args.eval_interval == 0:
            if hasattr(eval_env, "set_global_step"):
                eval_env.set_global_step(i)
